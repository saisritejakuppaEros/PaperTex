...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{images/final_abstract.png}
...
Mixture-of-Experts adapters and composable diffusion methods address this through signal routing or score composition, but typically at the cost of inference complexity or reduced per-condition fidelity.
EasyControl~\cite{zhang2025easycontrol} handles multi-condition coordination through its position-aware training paradigm and KV cache mechanism, which separate spatial and subject signals without mutual interference.
...
In summary, the approach most closely related to ours is the combination of ControlNet-style structural conditioning with IP-Adapter-style appearance conditioning.
The distinguishing factors of our pipeline are: (i) constant-cost multi-reference encoding via canvas compression; (ii) coarse layout induction from a composition prompt, avoiding a dedicated layout model in our implementation; and (iii) the use of EasyControl's LoRA-based DiT conditioning rather than UNet-specific adapters, enabling compatibility with modern Flux-based architectures.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{images/method.jpg}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{lcccccccc}
    ...
    & \multicolumn{4}{c}{\textbf{Global-wise Quality}} \\
    ...
    \textbf{RefCompose} ($N{>}5$)
      ...
      & \textbf{0.231} & \textbf{0.625} & \textbf{0.778} & 0.329 \\
    \bottomrule
  \end{tabular}%
  }
\end{table*}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{images/abalation.jpg}
