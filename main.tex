...
\begin{center}
  ...
  \captionof{figure}{Existing approaches suffer from memory overhead scaling linearly with reference count, cultural fidelity gaps, or inability to preserve subject appearance without bounding-box supervision. RefCompose resolves these via a fixed-resolution reference canvas encoding arbitrary references at constant token cost, and layout induced directly from a structured text prompt via dense visual cues.}
...
Recent advances in reference-based generation have shown promise in scaling to multiple subjects, bridging gaps in cultural and ethnic fidelity that text-only methods leave unaddressed~\cite{wu2025uno,he2025dreamo,xiao2024omnigen,omnigen2_2025,wang2024instantid,li2024photomaker}. However, encoding and injecting reference images introduces fundamental bottlenecks. Transformer-based diffusion models represent conditioning signals as token sequences, so when reference images are encoded independently and concatenated, token budget grows linearly with the number of references. Beyond four to six images, memory cost becomes prohibitive, and the problem compounds for high-resolution inputs~\cite{ye2023ip,wu2025uno,xverse2024,xiao2024omnigen,omnigen2_2025}. Attention-based alternatives such as QKV injection offer zero-shot control but remain spatially imprecise: appearance features leak across object boundaries, fidelity degrades for fine-grained subjects, and the inversion-then-manipulation pipeline reconstructs subjects that deviate substantially from the reference, making QKV injection fragile even at conservative noise levels~\cite{hertz2022prompt,mokady2023null,tumanyan2023plug}. Decoupling token complexity from the number of references while preserving spatial precision and fidelity thus remains a critical, unresolved challenge.

The core insight of this work is that token-scaling is a consequence of the chosen representation, not an inherent property of multi-reference generation. Instead of encoding each image separately, we compress a varying number of reference images onto a single fixed-resolution reference canvas in high-definition pixel space. Since the canvas size is fixed, downstream token count stays constant regardless of reference count, shifting the problem from sequence scaling to spatial composition (Refer Figure \ref{fig:conceptdiagram}).

Spatial composition requires knowing where each subject belongs. In RefCompose, this comes from a coarse prompt-induced layout generated by the frozen diffusion transformer, from which a monocular depth map is derived for geometric guidance without extra supervision. The reference canvas and depth map act as spatially aligned, complementary signals encoding appearance and structure respectively, injected via a LoRA-based conditioning framework for Diffusion Transformers that enables parameter-efficient adaptation without compromising the base model's generative capacity. Our contributions:
...
IP-Adapter~\cite{ye2023ip} decouples image and text conditioning via cross-attention, enabling subject-driven generation from a single reference.
Follow-up methods share a common structure: each reference is encoded independently and injected as an additional token sequence~\cite{wang2024instantid, li2024photomaker, ruiz2023dreambooth, kumari2023multiconcept}, so token cost grows with reference count.
Architectures concatenating multiple VAE encodings make this explicit: beyond a small number of references, GPU memory overflows~\cite{ye2023ip, wang2024msdiffusion, wu2025uno, xverse2024, xiao2024omnigen, omnigen2_2025}.
...
Spatial control has been pursued via layout priors of various kinds~\cite{zhang2023adding, li2023gligen, feng2024layoutgpt, lian2023layoutdiffusion, tang2024creatilayout, xiang2025instanceassemble}.
LayoutGPT~\cite{feng2024layoutgpt} uses an LLM to generate bounding boxes for conditioning, while Layout Diffusion models spatial arrangement distributions directly.
GLIGEN~\cite{li2023gligen} injects grounding tokens at specified locations via gated self-attention for object-level placement.
These add pipeline complexity; we find an explicit layout component unnecessary—state-of-the-art DiTs can use a generic composition prompt for coarse layout sufficient for canvas placement and depth extraction, though comparison against specialized layout generators remains future work.

\paragraph{Depth and structural conditioning.}
ControlNet~\cite{zhang2023adding} showed pre-computed spatial signals (depth, canny edges, pose) provide strong structural guidance for UNet-based diffusion via a trainable parallel encoder.
Adapting this to DiTs is non-trivial given the architectural shift from UNet.
EasyControl~\cite{zhang2025easycontrol} bridges this gap with a lightweight Condition Injection LoRA integrating spatial conditioning into DiT attention via causal attention and KV caching, achieving plug-and-play control without retraining.
We build on EasyControl as the conditioning backbone, using depth maps from layout-induced generations for geometric guidance while leaving base weights intact.
...
Compared to attention manipulation strategies like QKV injection or self-attention swapping—sensitive to target object footprint and inconsistent for small subjects~\cite{hertz2022prompt, mokady2023null, tumanyan2023plug}—LoRA offers more stable transfer.
We find inversion-based pipelines fragile: above 0.5 noise strength, structural distortion and fidelity loss become severe.
...
Most closely related is Canvas-to-Image~\cite{dalva2025canvastoimage}, which performs compositional generation from a multimodal canvas but lacks explicit orientation control and degrades in densely cluttered scenes.
Mixture-of-Experts adapters and composable diffusion address multi-signal conditioning via routing or score composition, typically at the cost of inference complexity or per-condition fidelity.
We instead adopt a disentangled design: EasyControl~\cite{zhang2025easycontrol} separates spatial and subject signals via position-aware training and KV caching, without mutual interference.
Our method exploits this to jointly inject depth (structure) and canvas (appearance) within a single forward pass.
...
\begin{center}
  ...
  \captionof{figure}{Overview of RefCompose: prompt-induced layout generation, correspondence-guided canvas construction, and decoupled structure--appearance conditioning via dual LoRA streams in the MMDiT blocks, preserving full-resolution identity at constant token complexity.}
  \label{fig:pipeline}
\end{center}
...
$\mathcal{F}_\theta$ is the frozen diffusion transformer; $\mathcal{E}, \mathcal{E}^{-1}$ the VAE encoder/decoder.
...
A single text-conditioned forward pass yields a coarse layout image:
\begin{equation}
...
Because depth $\mathcal{D}_s=\Psi(L)$ (Section~\ref{sec:depth}) and canvas assembly (Section~\ref{sec:canvas}) both read from the same $L$ and $\mathcal{B}$, geometric and appearance conditioning share one spatial frame---avoiding misalignment that would arise if layout, boxes, and depth were computed independently.
...
\end{equation}
using Depth Anything 3~\cite{depthanything3}. Deriving $\mathcal{D}_s$ from $L$ (rather than the references) ensures $L$, $\mathcal{B}$, $\mathcal{D}_s$ share an exact spatial frame. Since $\Psi$ runs on generated layouts, not real photos, we treat its output as a relative geometric prior, not metric ground truth.

For each reference $k$, we compute mean depth $\bar{d}_k = \mathrm{mean}(\mathcal{D}_s \mid b_k)$ and sort far-to-near as permutation $\pi$. The canvas is assembled by sequential overwrite compositing:
\begin{equation}
  \mathcal{C} =
  \mathrm{Paste}\!\left(
...
$\mathcal{D}_s$ is VAE-encoded for compatibility with latent denoising:
\begin{equation}
...
%----------------------------------------------------------
Naively concatenating $z_{\mathcal{D}}$ and $z_{\mathcal{C}}$ lets depth features leak into appearance and vice versa. We instead route each through a dedicated low-rank module.
...
\begin{equation}
  ...
  m \in \{s,a\},
\end{equation}
with $s$ the structure branch ($z_{\mathcal{D}}$) and $a$ the appearance branch ($z_{\mathcal{C}}$). Effective projection: $W + \Delta W_s + \Delta W_a$, base $W$ frozen.

Since Flux-style DiTs use flow-matching, denoising is scheduler-agnostic.
Starting from terminal latent $z_T$, the sampler integrates the velocity
field over the scheduler time grid to obtain
\begin{equation}
  \hat{z}_0 = \mathcal{F}_\theta\bigl(\mathbf{p}, z_T, z_{\mathcal{C}},
  z_{\mathcal{D}}; \AdaptW\bigr),
\end{equation}
where $\mathbf{p}$ provides semantic grounding, $z_{\mathcal{D}}$ the
geometric scaffold, and $z_{\mathcal{C}}$ localised appearance, and
\begin{equation}
  v_\theta\bigl(z_t, t, \mathbf{p}, z_{\mathcal{C}}, z_{\mathcal{D}};
  \AdaptW\bigr)
\end{equation}
denotes the learned velocity at denoising step~$t$. Output via VAE decoding:
\begin{equation}
...
\textbf{Dataset Construction.} We train on ultra high resolution cinematic imagery at $1280 \times 720$ resolution, drawn from Eros's proprietary movie catalogue, where most object references are canonicalized to a neutral pose. We complement this with a synthetic dataset generated using Flux 2.0, providing diverse object positions and orientations so the model learns pose variation while the real dataset supplies richer textures; further details are provided in the supplementary material.

Our pipeline proceeds in five stages. First, each movie shot is passed through Qwen3-VL~\cite{qwen3vl} to enumerate all objects present in the scene---actors, props, and landmarks. Second, Grounding DINO~\cite{liu2023grounding} takes these object descriptions as text prompts to detect corresponding bounding boxes, retaining only shots yielding more than four detections to ensure compositional density. Third, each detected crop is fed to Flux~2.0 Dev to synthesize a neutral-pose, neutral-lighting canonicalization, removing scene-specific pose and illumination variation. Fourth, we filter canonicalized outputs against their original crops using DINO cosine similarity, retaining a sample only if similarity exceeds $0.90$. Finally, retained shots are annotated again with Qwen3-VL, producing both scene-level and crop-level descriptions (Figure~\ref{fig:data_pipeline}). This pipeline yields approximately $50{,}000$ samples drawn from $1{,}000$ distinct films, together with additional synthetic samples.
\begin{center}
  \includegraphics[width=\linewidth]{images/dataset_prep_compressed.png}
  \captionof{figure}{Overview of the dataset preparation pipeline. Objects are identified using Qwen3-VL, localized with Grounding DINO, canonicalized to a neutral pose and lighting using Flux~2.0 Dev, filtered via DINO similarity, and finally annotated by Qwen3-VL.}
...
We instantiate the layout pipeline of Section~\ref{sec:layout} at $1280 \times 720$ with Flux~2.0 Dev as $\mathcal{F}_\theta$.
...
\end{quote}
Slots may encode relational placement (\emph{e.g.}, left/right, foreground/background) when metadata supports it.
Appearance attributes are omitted from subject slots---reference crops supply pixels, and duplicating appearance in $\mathbf{p}$ would confound text- versus canvas-driven conditioning.
...
All methods run on a single NVIDIA H100 at \(1280 \times 720\) using the prompt template from Section~\ref{sec:layout-gen}. We select five object crops per image from the largest ground-truth boxes by area, canonicalize each via Flux~2.0 Dev (neutral pose/lighting), and generate scene prompts with Qwen3-VL~\cite{qwen3vl}, ensuring all methods receive identical textual and visual information. CreatiLayout and InstanceAssemble~\cite{xiang2025instanceassemble} receive only Dense Layout boxes and text; UNO~\cite{wu2025uno}, XVerse~\cite{xverse2024}, and RefCompose additionally receive the canonicalized reference crops. Region-wise metrics use object localizations from Grounding DINO~\cite{liu2023grounding}. As Table~\ref{tab:memory} shows, token-based methods grow in memory with reference count and go out-of-memory at \(N=6\), while RefCompose stays approximately constant via canvas compression. Baselines are thus reported at \(N=5\), with Table~\ref{tab:quant_results} also reporting RefCompose at \(N>5\) to demonstrate scalability without added memory cost.

Following LayoutSAM-Eval~\cite{xiang2025instanceassemble}, we report \emph{region-wise} quality (Spatial, Color, Texture, Shape — placement accuracy and appearance/structure fidelity) and \emph{global-wise} quality (PICK, CLIP-T for image-text alignment; DINO for composition similarity; DINO$_{\text{refs}}$ for reference consistency; CLIP-I for image-level similarity). Higher is better throughout.
...
\caption{Ablation study on an indoor multi-object scene. Removing either the reference canvas or depth prior degrades identity preservation or geometric consistency, while the full model preserves object appearance and spatial layout by leveraging both conditioning signals.}
\label{fig:ablation}
\end{figure}
